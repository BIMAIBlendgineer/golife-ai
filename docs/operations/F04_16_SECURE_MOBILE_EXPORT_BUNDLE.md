...
- HomeMemory submission files now copy into an app-private vault and persist only an internal managed reference in `PurchaseProof.fileRef` and `EvidenceAttachment.fileRef`.
...
- PR branch green on `flutter` job
